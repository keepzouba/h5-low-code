...
export const Editor: React.FC<IProps> = ({
  ...
  const [componentPannelVisible, setComponentPannelVisible] =
  ...
  const [components, setComponents] =
    useState<Array<IComponent>>(defaultComponents);
  ...
  const addComponent = (component: IComponent) => {
    setComponents((components) => {
      return [...components, { ...component, id: "" + components.length }];
    });
  };
  ...
  const copy = (index: number) => {
    setComponents((components) => {
      ...
      target.props = target.defaultProps as any;
  ...
  // 编辑当前组件 props
  const editCurrentComponentProps = (newProps: any) => {
    setComponents((components) => {
  ...
  // 交换两组件位置（拖拽）
  const swapComponent = (startIndex: number, endIndex: number) => {
    setComponents((components) => {
  ...
  const save = () => {
    ...
    const data = components.map((component: any) => {
      return {
        ...
        fns: component.fns.map(
          (fn: { name: any; props: any; schema: any }) => ({
            name: fn.name,
            props: fn.props,
            schema: fn.schema,
          })
