...
export const Pannel: React.FC<IProps> = ({
  ...
  const groups = EditorPlugins.components.getGroups();
  const [group, setGroup] = useState(groups[0]);

  useEffect(() => {
    // console.log('pluginsManager',pluginsManager);
    
  }, []);


  const components = useMemo(() => {
    return (EditorPlugins.components.getObjsByGroup(group) || []).filter(
      Boolean
    );
  }, [group]);

  console.log("groups", groups, EditorPlugins.components);
